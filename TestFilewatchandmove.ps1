...
$file='\\172.31.26.44\XferPush\Choctaw\Choctaw-ImportData.csv'
$Database = "CastleAnalytics"
$Server = "Castle Hill AWS server"
$Port = "5432"


if(Test-Path $file){

#New-item -Path C:\XferData\ChoctawData\Logging1.txt -Value 'First entry '
  $Target1 = "C:\XferData\ChoctawData\Choctaw-ImportData.csv"
  $Target2 = "C:\XferData\ChoctawData\Choctaw-ImportData" + $(get-date -f yyyy-MM-dd) + ".csv"
	Copy-Item "\\172.31.26.44\XferPush\Choctaw\Choctaw-ImportData.csv" -Destination $Target1
	Copy-Item "\\172.31.26.44\XferPush\Choctaw\Choctaw-ImportData.csv" -Destination $Target2
    Remove-Item $file
#$conn = Connect-PostgreSQL  -User "$User" -Password "$Password" -Database "$Database" -Server "$Server" -Port "$Port"
...
{
#New-item -Path C:\XferData\ChoctawData\Logging2.txt -Value 'End of file '
}
